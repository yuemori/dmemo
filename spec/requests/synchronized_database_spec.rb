...
describe :synchronized_database, type: :request do
  ...
  describe "#update" do
    it "synchronize data_source" do
      ...
      expect(dataset.columns.map(&:name)).to match_array(%w(id name description adapter bigquery_dataset_name bigquery_project_id bigquery_keyfile host port dbname user password encoding pool created_at updated_at type))
      expect(dataset.rows.take.row[1..10]).to match_array(["dmemo", "", "", "", "", data_source.adapter, "localhost", "5432", data_source.dbname, data_source.user])
